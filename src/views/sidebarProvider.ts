...
export class SidebarProvider implements vscode.WebviewViewProvider {
  ...
  private _buildArchitectureInstruction(customPrompt: string = ''): string {
    const prompt = [
      ...
      `  * box (generic processes/actions)`,
      `  * database (databases, storage)`,
      `  * cloud (internet, network, CDN, external services)`,
      `  * server (servers, infrastructure, hosts)`,
      `  * browser (client browsers, web UIs)`,
      `  * component (services, microservices, modules)`,
      `  * cylinder (message queues, buffers, brokers)`,
      `  * rounded-rect (APIs, gateways, interfaces)`,
      `  * pill (start/end terminals)`,
      `  * diamond (decisions, conditions)`,
      `  * mobile (mobile apps)`,
      `  * document (documents, reports)`,
      `  DO NOT default everything to 'box'. Use the MOST SEMANTICALLY APPROPRIATE shape for each entity.`,
  ...
  private _buildDiagramInstruction(customPrompt: string = ''): string {
    const prompt = [
      ...
      ``,
      `Rules:`,
      `- Use the MOST SEMANTICALLY APPROPRIATE shape type for each entity:`,
      `  * Decision/condition nodes -> type "diamond"`,
      `  * Start/End nodes -> type "pill"`,
      `  * Generic process/action -> type "box"`,
      `  * Database/storage -> type "database"`,
      `  * Cloud/Internet/Network/CDN -> type "cloud"`,
      `  * Server/Infrastructure/Host -> type "server"`,
      `  * Client/Browser/Web UI -> type "browser"`,
      `  * Queue/Message Broker/Buffer -> type "cylinder"`,
      `  * Service/Microservice/Module -> type "component"`,
      `  * API/Gateway/Interface -> type "rounded-rect"`,
      `  * Mobile/App -> type "mobile"`,
      `  * Document/Report -> type "document"`,
      `  DO NOT default everything to 'box'. Use visually distinct shapes.`,
      `- Nodes that represent components must have a unique id (e.g. "node_0", "node_1")`,
      `- Nodes that represent connections must have startConnection and endConnection referencing those ids`,
